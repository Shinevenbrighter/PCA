...
def verificar_modelo():
    ...
    if pendientes.count() >= 20: entrenar_pca()

def entrenar_pca():

    jugadores = list(Jugador.objects.filter(entrenado=False).order_by('id')[:20])

    X = np.array([jugador.vector_pca()for jugador in jugadores])

    if os.path.exists(MODEL_PATH): pca = joblib.load(MODEL_PATH)

    else: pca = IncrementalPCA(n_components=2, batch_size=20)
